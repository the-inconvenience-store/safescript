...
type HandlerTable<O extends Operations, C> = {
...
export function createSafeScript<C, O extends Operations, S extends Slots, E extends PolicyError = PolicyError>(
  ...
  const operationsById = new Map(
    operationEntries.map(([key, operation]) => [operation.id, { key, operation }] as const),
  );
    ...
    return {
      async handleAction(request: ActionRequest): Promise<ActionOutcome> {
        ...
        const decoded = decodeCanonical(
          { kind: 'ref', type: entry.operation.input.id },
          Uint8Array.from(request.input),
          { registry: options.contract.registry.schemas },
        );
  ...
  return freeze({
    ...
    execute(request: ExecuteRequest<PropertyKey, unknown, C>): Promise<ExecutionResult<unknown>> {
      ...
      return run(async () => {
        ...
        try {
          return await executeBridge(slot, request, gateway(request.context, slot, controller.signal), invocationId);
        } finally {
      ...
      return run(
        async () => {
          ...
          if (!slot)
            return freeze({
              passed: false,
              mismatches: [mismatch('slot', request.slot, undefined)],
          ...
          const host: RuntimeBridgeHost = {
            async handleAction(action: ActionRequest): Promise<ActionOutcome> {
              ...
              const entry =
                typeof script.operation === 'string' && script.operation in options.contract.operations
                  ? options.contract.operations[script.operation]
          ...
          if (expected?.effects !== undefined) {
            const actual =
              execution.status === 'completed' || execution.status === 'failed' || execution.status === 'cancelled'
                ...
                : [];
            if (stable(expected.effects) !== stable(actual))
              mismatches.push(mismatch('effects', expected.effects, actual));
          }
          if (expected?.actions !== undefined) {
            const actual =
              execution.status === 'completed' || execution.status === 'failed' || execution.status === 'cancelled'
                ? execution.facts.actions
                : [];
            if (stable(expected.actions) !== stable(actual))
              mismatches.push(mismatch('actions', expected.actions, actual));
          }
