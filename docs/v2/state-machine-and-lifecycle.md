...
The host reserves response capacity before accepting the action, validates canonical input and registry metadata, derives current resource scope, performs current authorization, and dispatches at most one handler. It returns exactly one `action.outcome` containing a completed declared result, typed policy rejection, or host failure with explicit effect state.
