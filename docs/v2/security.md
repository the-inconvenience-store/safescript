...
This document extends the [SafeScript security model](../security.md) across the v2 process boundary. Process separation is defense in depth; it does not replace SafeScript language restriction, IR verification, canonical validation, metering, or current host authorization.
...
The runtime worker contains the compiler, verifier, and interpreter, but the host adapter treats it as a protocol-untrusted peer. Worker compromise must not directly expose host credentials, handlers, authorization state, arbitrary host objects, or ambient authority. The worker likewise treats the host as a potentially malformed peer and validates every frame before work.
...
Handlers, credentials, invocation context, resource-scope evaluation, current authorization, external idempotency enforcement, and effect dispatch remain exclusively in the host process. They MUST NOT be serialized to the worker.

Each worker `action.request` is only a typed proposal. The host validates session and execute correlation, ABI and contract facts, operation/effect/capability/action-site identities, canonical input, idempotency facts, and current resource scope. It then calls current authorization immediately before at-most-once handler dispatch. Static summaries, successful handshake, checked artifacts, prior authorization, and process identity never grant authority.
...
These controls MUST NOT weaken protocol validation or current authorization. The SDK documentation MUST distinguish tested recipes from portable requirements.

## Security non-goals

V2 does not claim that a plain Node child process denies filesystem, network, or syscalls after worker compromise. It does not sandbox trusted handlers, protect against an over-permissive authorization callback, hide source from the worker, make action effects reversible, or provide durable recovery.
