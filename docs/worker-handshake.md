...
The worker compares the hello with its exact release, worker/compiler identity, and limits. A compatible worker replies once with `session.welcome`, containing `version`, worker identity, selected limits, and implementation name. Each selected limit is the minimum of the host request, worker maximum, and built-in ceiling.
